...
ax = predictions_df_.rename(columns={"Prices": "predicted_price"}).plot(title=company_name+' Linear Regression predicted prices')#predicted value
ax.set_xlabel("Indexes")
ax.set_ylabel("Stock Prices")
fig = y_test.rename(columns={"Prices": "actual_price"}).plot(ax = ax).get_figure()#actual value
fig.savefig(company+"_Linear_Regression.png")

# Save
import gzip
with gzip.GzipFile(company+'_Linear_Regression_model.pgz', 'w') as f:
    pickle.dump(clf,f)
print("Model saved!")
with gzip.GzipFile(company+'_Linear_Regression_Score.pgz', 'w') as e:
