...
get_date=new_dict.Date.iloc[len(new_dict)-1]
temp_df = pd.DataFrame([[get_date, 
                                '0']], columns = ['Date','trend'])
aaadata = pd.concat([aaadata, temp_df], axis = 0).reset_index(drop = True)        
...
train_Company=new_dict[['Date','Prices','Comp','Negative','Neutral','Positive','last_close','vol','trend']].copy()
print(train_Company)
...
knn.fit(numpy_df_train, y_train)
prediction=knn.predict(numpy_df_test)
print(prediction)
...
# Save
import gzip
with gzip.GzipFile(company+'_KNN_model_more_trend.pgz', 'w') as f:
    pickle.dump(knn,f)
print("Model saved!")
with gzip.GzipFile(company+'_KNN_more_trend_Score.pgz', 'w') as e:
    pickle.dump(acc,e)