...
ax = predictions_df_.rename(columns={"Prices": "predicted_price"}).plot(title=company_name+' Linear SVR predicted prices')#predicted value
ax.set_xlabel("Indexes")
ax.set_ylabel("Stock Prices")
fig = y_test.rename(columns={"Prices": "actual_price"}).plot(ax = ax).get_figure()#actual value
fig.savefig(company+"_Linear_SVR.png")

# Save
import gzip
with gzip.GzipFile(company+'_Linear_SVR_model.pgz', 'w') as f:
    pickle.dump(regr,f)
print("Model saved!")
with gzip.GzipFile(company+'_Linear_SVR_Score.pgz', 'w') as e:
